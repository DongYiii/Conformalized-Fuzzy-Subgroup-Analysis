...
# Create y
# The true values of beta are (0.5,0.5)^T, (0.7,0.7)^T and (0.7,0.4)^T
y = rep(0,n) # y为n个0
for (i in 1:n/3) {
  y[i] = 0.5*x1[i]+0.5*x2[i]+ep[i]
}
for (i in (1+(n/3)):(2*n/3)){
  y[i] = 0.7*x1[i]+0.7*x2[i]+ep[i]
}
for (i in (1+(2*n/3)):n){
  y[i] = 0.7*x1[i]+0.4*x2[i]+ep[i]
}
...
for (i in 1:length(beta1)) {
  for (j in 1:length(beta2)) {
    for (k in 1:n) {
      ...
      if(abs(resij[i,j,k])<0.1) {resij[i,j,k] = 0}
      else {resij[i,j,k] = 1}
    }
  }
}
...
for (i in 1:length(beta1)) {
  for(j in 1:length(beta2)){
    ...
    if(rsd_ratio[i,j] > 0.33){Ps[i,j] = 1}
  }
}
...
for(i in 2:length(xax)){
  if(yax[i]-yax[1]<0.2){
    subgroup1_x=c(subgroup1_x,xax[i])
    subgroup1_y=c(subgroup1_y,yax[i])
    types[i]=2
    ind=i+1
  }
  else {
...
persp(beta1,beta2,rsd_ratio,col = color[facetcol], phi = 25, theta = 32 ,xlab = '', ylab = '', zlab = '', main = NULL, sub = NULL, shade = 0.1)
